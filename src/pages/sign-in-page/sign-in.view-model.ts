...
import { filter, map, mapTo, pluck, shareReplay, switchMap, tap } from 'rxjs/operators';
...
import { createAdapter } from '../../utils/rx.utils';
...
export const createSessionViewModel = (): SignInViewModel => {
	...
	const loginRequest$ = login$.pipe(
		...
		switchMap(({ username, password }) => login(username, password).pipe(tap(handleLoading(false)), shareReplay())),
	);

	const userDetailsFromLogin$ = loginRequest$.pipe(
	...
	const userDetailsFromLogout$ = logout$.pipe(
		switchMap(logout),
		filter(isRequestSuccessful),
		mapTo(option.none),
		shareReplay(),
	...
	const userDetails$ = merge(userDetailsFromLogin$, userDetailsFromLogout$).pipe(shareReplay());
